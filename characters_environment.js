...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona el botón para jugar", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
{
  GameStatus = "start";
  document.getElementById("estatus").innerHTML = "El juego está cargando";
}

// Cambiar el estado del juego si se presiona cualquier tecla
function changeGameStatud(character){
  console.log("el juego cargo");
 if(noseX !="" && gameConfig.status==="start" && GameStatus=="start") { 
   document.getElementById("estatus").innerHTML = "El juego se ha cargado";
   console.log("si cargo :D");
   world_start.play();
